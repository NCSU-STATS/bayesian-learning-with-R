...
stacks_dat <- list(Y1=Y1,Y2=Y2,Y3=Y3,p = 6,   N = 365)

mlr_model <- function(){
  
  #See appendix - this part causes instabilities in OpenBUGS.  We fit rho and Sigma2 in jags below.
  # Y1pred[1,1:p] ~ dmnorm( theta[1,1:p] ,precision[,])
  # 
  # for(i in 2:N) {
  #   Y1pred[i,1:p] ~ dmnorm( var1[i,1:p] ,precision[,])
  # 
  #   var1[i,1]<-  mu2[1] + rho * theta[i-1,1] 
  #   var1[i,2]<-  mu2[2] + rho * theta[i-1,2]
  #   var1[i,3]<-  mu2[3] + rho * theta[i-1,3]
  #   var1[i,4]<-  mu2[4] + rho * theta[i-1,4]
  #   var1[i,5]<-  mu2[5] + rho * theta[i-1,5]
  #   var1[i,6]<-  mu2[6] + rho * theta[i-1,6]
  # }
  # 
  # rho  ~  dunif(0,1)
  # 
...
model
{
  ...
  for(i in 2:N) {
    Y1pred[i,1:p] ~ dmnorm( nu[i,1:p] ,precision[,])
    
    nu[i,1]<-  mu2[1] + rho * theta[i-1,1]
    nu[i,2]<-  mu2[2] + rho * theta[i-1,2]
    nu[i,3]<-  mu2[3] + rho * theta[i-1,3]
    nu[i,4]<-  mu2[4] + rho * theta[i-1,4]
    nu[i,5]<-  mu2[5] + rho * theta[i-1,5]
    nu[i,6]<-  mu2[6] + rho * theta[i-1,6]
    }
...
'
model_data = list(theta=theta,Y2=as.vector(scale(Y2.test)),Y3=as.vector(scale(Y3.test)),N = N, p = p)
