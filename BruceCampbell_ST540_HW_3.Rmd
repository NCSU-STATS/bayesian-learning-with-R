...
for(i in 1:nrow(NBA_Data))
{
  ...
  NBA_Data[i,]$posteriorProbTest <- pbeta(p,a,b)
  
  NBA_Data[i,]$testResult <-NBA_Data[i,]$posteriorProbTest>=0.1
  
}

pander(data.frame(name = NBA_Data$PlayerName, posterior.probability = NBA_Data$posteriorProbTest, test.result =NBA_Data$testResult))
```
...
for(i in 1: nrow(NBA_Data))
{
  ...
  NBA_Data[i,]$uninformativePosteriorProbTest <- pbeta(p,posteriorA,posteriorB)
  
  NBA_Data[i,]$uninformativeTestResult <-NBA_Data[i,]$uninformativePosteriorProbTest>=0.1
  
}

pander(data.frame(name = NBA_Data$PlayerName, posterior.probability = NBA_Data$uninformativePosteriorProbTest, test.result =NBA_Data$uninformativeTestResult))
