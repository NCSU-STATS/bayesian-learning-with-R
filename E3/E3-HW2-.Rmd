...
'
  ...
  model_pois_lattitude <- jags.model(textConnection(model_pois_lattitude),data = model_data,n.chains = n.chains)#Compile Model Graph
  update(model_pois_lattitude, nSamples, progress.bar="none"); # Burnin
  out.coda  <- coda.samples(model_pois_lattitude, variable.names=model_parameters,n.iter=2*nSamples,,n.thin=2)
  #plot(out.coda)
  ...
  colnames(pval.m)<-c("pval.mean","pval.sd")
  pander(data.frame(pval.m), caption = "Baeysian p-values Poisson Lattitude GLM")
...
dic_pois_lattitude  <- dic.samples(model_pois_lattitude,variable.names=c("beta", "alpha"), n.iter=nSamples, progress.bar="none")
dic_pois_lattitude
```
