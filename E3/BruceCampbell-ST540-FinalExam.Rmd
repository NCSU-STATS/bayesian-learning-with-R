...
Once we established working JAGS models, we ran a short run of the chains and calculated the Raftery and Lewis diagnostic.  This is a run length diagnostic intended to provide insight on how long to run a chain.  The diagnostic estimates the number of iterations required to estimate the quantile q to within an accuracy of +/- r with probability p.  We ran the chains for 1000 iteration, and the diagnostic indicated running the chains for 3746 iterations using $q=0.25$, $r = +/- 0.005$, and $p=0.95$. We choose to run the models for 20,000 iterations with a thinning of 2. 
...
The effective sample size (ESS) is a diagnostic describing the sample size of the MCMC chain adjusted for auto-correlation.  Strong auto-correlation reduces the ESS.  We accommodate by running longer chains and increasing the thinning.  The convergence diagnostics for the Poisson GLM model fit to the WMO heatwave count data is displayed in figure 2. We did note that the ESS of some of the chains had a low value.  We subsequently ran the MCMC chain for 100,000 iteration to see if we could get consistent ESS values for all the tracked nodes but we did not. The low ESS was observed on the intercept and slope coefficients of the GLM which do have significant auto-correlation out to lag 15-20 which could explain the low observed ESS values.  We increased the thinning to 10 in order to accommodate the auto-correlation.      
...
The jags code was instrumented to provide predictions which were used calculate the MSE on the training data. The DIC was also evaluated to compare models. The Poisson is a limiting distribution for the negative Binomial.  The DIC for the 2 models and the training MSE are very close.  We chose the negative binomial model based on its simplicity and slightly lower DIC.  

-------------------------------------------
Deviance For WMO Heat Wave Data
-------------------------------------------
Poisson Mean deviance:  957.4 

Poisson penalty 16.68 

Poisson Penalized deviance: 974.1 

negative binomial Mean deviance:  957.6 

negative binomial penalty 19.26 

negative binomial Penalized deviance: 976.9 
-------------------------------------------
...
We noted that the 2 cities that were significant for a positive slop in the GLM were more northern than the other cities in the data.  We attempted to add latitude as a predictor in the GLM, but the coefficients for that feature did not come out significant.
