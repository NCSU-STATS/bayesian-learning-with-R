...
interncept.max.mode <- which.max(intercepts.means.modes$mode)

pander(data.frame(intercept.max.mean.index=villages[intercept.max.mean], interncept.max.mode.index=villages[interncept.max.mode]), caption="Village index of max mean and mode for intercept posteriors")
...
pander(data.frame(slope.max.mean.index=villages[slope.max.mean], interncept.max.mode.index=villages[slope.max.mode]), caption="Village index of max mean and mode for slope posteriors")
```


### Plot mean intercept and slope by village

```{r}
plot(slopes.means.modes$mean)
```

### Calculate credible intervals and widths for 95% credibile intervals which don't contain zero.
...
The MCMC algorithm converged for all the parameters. We saw evidence that the slopes and intercepts vary by village. We plot the proportions of net use and positive malaria status below and compare that to the results above. 



```{r}
#There's a bug in sql df 
# library(sqldf)
# 
# counts.pos <- data.frame(sqldf("select village, count(*) as pos from df where pos ==0 group by village"))
# 
# counts.net <- data.frame(sqldf("select village, count(*) as net from df where net ==1 group by village"))
# 
# counts.village <- data.frame(sqldf("select village, count(*) as count from df group by village"))
# 
# proportions <- cbind(counts.pos,counts.net$net,counts.village$count)
...
acc<- data.frame(accuracy =rep(NaN,65))
for(i in 1:65)
{
  t <- tables[,,i]
  total <- t[1,1]+t[1,2]+t[2,1]+t[2,2]
  tp <- t[2,2]
  tn <- t[1,1]
...
plot(acc$accuracy, main="accuracy by village")

villages[which.max(acc$accuracy)]
```
